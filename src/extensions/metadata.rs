...
use crate::client::*;
use crate::error::{Error, Result, ParseError};
use crate::types::*;
use std::io::{Read, Write};
use imap_proto::types::{Metadata, Response, MailboxDatum};
use std::sync::mpsc;
...
impl CmdListItemFormat for Metadata
{
    fn format_as_cmd_list_item(&self) -> String
    {
        format!("{} {}", validate_str(self.entry.as_str()).unwrap(),
                self.value.as_ref().map(|v| validate_str(v.as_str()).unwrap()).unwrap_or("NIL".to_string()))
    }
}
...
fn parse_metadata<'a>(mut lines: &'a [u8],
    ...
    loop
    {
        ...
        match imap_proto::parse_response(lines)
        {
            Ok((rest, resp)) => {
                lines = rest;
                match resp {
                    Response::MailboxData(MailboxDatum::MetadataSolicited{mailbox: _, mut values}) =>
                    {
                        res.append(&mut values);
                    },
                    Response::MailboxData(MailboxDatum::MetadataUnsolicited{mailbox, values}) =>
                    {
                        unsolicited.send(UnsolicitedResponse::Metadata{mailbox: mailbox.to_string(),
                                                                       metadata_entries: values.iter().map(|s| s.to_string()).collect()}).unwrap();
                    },
                    _ => {},
                }
            },
            Err(_) => { return Err(Error::Parse(ParseError::Invalid(lines.to_vec()))); }
        }
    }
}

/// Sends GETMETADATA command to the server and returns the list of entries and their values.
pub fn get_metadata<'a, S: AsRef<str>, T: Read + Write>(session: &'a mut Session<T>,
                                          mbox: S, entries: &[S],
                                          depth: MetadataDepth,
                                          maxsize: Option<usize>) -> Result<Vec<Metadata>>
{
    let v: Vec<String> = entries.iter().map(|e| validate_str(e.as_ref()).unwrap()).collect();
    ...
    session.run_command_and_read_response(command).and_then(|lines| parse_metadata(&lines[..], &mut session.unsolicited_responses_tx))
}

/// Sends SETMETADATA command to the server and checks if it was executed successfully.
pub fn set_metadata<'a, S: AsRef<str>, T: Read + Write>(session: &'a mut Session<T>, mbox: S, keyval: &[Metadata]) -> Result<()>
{
    let v: Vec<String> = keyval.iter().map(|metadata| metadata.format_as_cmd_list_item()).collect();
    let s = v.as_slice().join(" ");
...
mod tests {
    ...
    fn test_getmetadata()
    {
        ...
        let r = get_metadata(&mut session, "", &["/shared/vendor/vendor.coi", "/shared/comment"],
                             MetadataDepth::Inf, Option::None);
